...
// Bản đồ di sản phiên bản DỌC (mobile) — bản mới bỏ làng Sơn (chỉ còn Quạt/Mộc/Tre).
// Giữ đúng public_id cũ "banner-di-san-mobile" để URL trong code không đổi.
const SOURCE_FILE =
  "C:\\Users\\ndcpr\\Downloads\\Logo + banner\\z7973803859379_33eb9ba36484215fa45c11aaf14bf132.jpg";
const PUBLIC_ID = "banner-di-san-mobile";
...
async function uploadAsset(filePath, publicId, config) {
  ...
  return { publicId: result.public_id, secureUrl: result.secure_url, width: result.width, height: result.height, version: result.version };
}
...
console.error(`✓ upload → ${uploaded.publicId} (${uploaded.width}x${uploaded.height}) version=${uploaded.version}`);
